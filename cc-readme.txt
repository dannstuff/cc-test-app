...
I separated  the UI into different categories, for example things I thought would be views and things I thought would be individual components etc.
I also planned to have a common 'Layout' component that renders all other views and would live at application level, as in a real production environment, 
...
A constant thought in my mind was to develop the page and application as if it were a real production application and there were others working on it, so I made sure to implement standards 
like code consistency, a consistent file/directory structure and attempted to make all components as lightweight as I possibly could. Initially, I had planned to use Redux and possibly some 
server-side rendering with Next JS, however I felt for the purpose of this singular page, using both would possibly slow me down and could have been overkill and more expensive than beneficial.
...
Unfortunately, I ran out of time to implement testing and instead decided to focus on getting as much of the UI done as I possibly could. As the application is pretty static,
the chances of anything breaking are extremely small, however I think in an actual production environment where you would be retrieving information from an API, more attention should be paid
